...
def load_dataset(seq_path, labels_path_minus, labels_path_plus, data_type):
    ...
    for i in range(len(features_list)):
        features_list[i] = np.insert(np.array(features_list[i], dtype=float), 4, lunps[i], axis=1)

    features = np.asarray(features_list, dtype=float)
...
def load_dataset_model_8_points(seq_path, labels_path_minus, labels_path_plus, data_type, only_predict=False,
                                split=True, index_for_split=None):
    ...
    if (only_predict == False):
        ...
        if (split == True):
            return_value = split_features_and_labels_for_8_points(features, initial_values_features, labels,
                                                                  index_for_split)
    else:
...
def split_features_and_labels_for_8_points(features, initial_values_features, labels, index_for_split):
    if (index_for_split is None):
        train_x, test_and_validation_x, train_x_initial, test_and_validation_x_initial, train_y, test_and_validation_y = train_test_split(
            features, initial_values_features, labels, test_size=0.22, random_state=31)
        test_x, validation_x, test_x_initial, validation_x_initial, test_y, validation_y = train_test_split(
            test_and_validation_x, test_and_validation_x_initial, test_and_validation_y, test_size=0.5, random_state=31)
        return_value = (train_x_initial, train_x, train_y), (validation_x_initial, validation_x, validation_y), (
        test_x_initial, test_x, test_y)
    else:
        ...
        return_value = (initial_values_features[train_ids], features[train_ids], labels[train_ids]), (
        initial_values_features[validation_ids], features[validation_ids], labels[validation_ids]), (
                       initial_values_features[test_ids], features[test_ids], labels[test_ids])
...
def load_dataset_linear_model(seq_path, labels_path_minus, labels_path_plus, data_type, only_predict=False, split=True,
...
def split_features_and_labels_for_linear(features, labels, index_for_split):
    if (index_for_split is None):
        train_x, test_and_validation_x, train_y, test_and_validation_y = train_test_split(features, labels,
                                                                                          test_size=0.22,
                                                                                          random_state=31)
        test_x, validation_x, test_y, validation_y = train_test_split(test_and_validation_x, test_and_validation_y,
                                                                      test_size=0.5, random_state=31)
        ...
        validation_ids = df['validation_ids'].dropna().astype(int).values
        test_ids = df['test_ids'].dropna().astype(int).values
        return_value = (features[train_ids], labels[train_ids]), (features[validation_ids], labels[validation_ids]), (
        features[test_ids], labels[test_ids])
...
###################################################################################
# load dataset_linear_model - for validation_seq- split=False, for train_validation_test - split=True
def load_dataset_model_type(seq_path, labels_path_minus, labels_path_plus, model_type, data_type, only_predict=False,
                            split=True, index_for_split=None):
    if (model_type == 'dynamics'):
        return load_dataset_model_8_points(seq_path, labels_path_minus, labels_path_plus, data_type, only_predict,
                                           split, index_for_split)
    elif (model_type == 'rate'):
        return load_dataset_linear_model(seq_path, labels_path_minus, labels_path_plus, data_type, only_predict, split,
                                         index_for_split)
    else:
