...
def model_creation_and_fit(train_set_wrapper, validation_set_wrapper, model_type, data_type, layers_params,
                           model_params, model_id_and_timestamp):
    ...
    if (model_type == 'rate'):
        ...
        # create input layer
        nodes_names_to_nodes.update({'input1': layers.Input(
            shape=(train_x.shape[1], train_x.shape[2]))})  # layers.Input(shape=(None, train_x.shape[2]))})
    ...
    if (model_params['gradient_clipping_norm'] is not None):
        optimizer = tf.keras.optimizers.get({"class_name": model_params['optimizer'],
                                             "config": {"learning_rate": model_params['lr'],
                                                        "clipnorm": model_params['gradient_clipping_norm']}})
    else:
        optimizer = tf.keras.optimizers.get(
            {"class_name": model_params['optimizer'], "config": {"learning_rate": model_params['lr']}})
    ...
    if (model_params['tensor_board'] == True):
        logdir = os.path.join(general_utilies.files_dir + "logs",
                              model_type + '_' + data_type + '_' + model_id_and_timestamp)
        callbacks.append(tf.keras.callbacks.TensorBoard(logdir, histogram_freq=1))
    if (model_params['early_stooping_patience'] > 0):
        callbacks.append(
            tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=model_params['early_stooping_patience'],
                                             min_delta=model_params['early_stooping_min_delta'], verbose=1,
    ...
    if (model_params['model_checkpoint'] == True):
        ...
        callbacks.append(
            tf.keras.callbacks.ModelCheckpoint(filepath, monitor='val_loss', verbose=0, save_best_only=False,
