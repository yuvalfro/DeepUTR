...
def predict_and_evaluate(model, set_wrapper, model_type, data_type, results, compute_mean_and_std=True,
                         only_predict=False, only_evaluate=None):
    # linear model need different treatment
    if (model_type == 'rate'):
        return linear_model_predict_and_evaluate(model, set_wrapper, data_type, results, compute_mean_and_std=True,
                                                 only_predict=only_predict, only_evaluate=only_evaluate)
    ...
    if (data_type == '-+'):
        print("A-")
        mertric_results = evaluation_utilies.evaluate_model(predicted_test[:, :output_size], y[:, :output_size],
...
def linear_model_predict_and_evaluate(model, set_wrapper, data_type, results, compute_mean_and_std=True,
                                      only_predict=False, only_evaluate=None):
...
def train_and_evalute(train_set_wrapper, validation_set_wrapper, test_set_wrapper, validation_seq_wrapper, model_type,
    ...
    model = NN_model.model_creation_and_fit(train_set_wrapper, validation_set_wrapper, model_type, data_type,
                                            layers_params, model_params, model_id_and_timestamp)
...
def train_test_validate_model_type(seq_path, labels_path_minus, labels_path_plus, model_type, data_type, model_id,
                                   ...
                                   train_spec=None, layers_spec=None, results_df_path=None, save_model_path=None,
    ...
    if validate_seq_path is not None:
        validation_seq_wrapper = NN_load_datasets.load_dataset_model_type(seq_path=validate_seq_path,
    ...
    model = train_and_evalute(train_set_wrapper, validation_set_wrapper, test_set_wrapper, validation_seq_wrapper,
                              model_type, data_type, model_id, train_spec, layers_spec, results_df_path,
                              save_model_path)
...
def evaluate_model_type(model_path, seq_path, labels_path_minus, labels_path_plus, model_id, model_type, data_type,
                        ...
                        compute_mean_and_std=True, index_for_split=None, preforme_IG_test=None,
                        preforme_TF_modisco=None,
                        GPU=True, only_predict=False, split=True, output_path=None):
    if (GPU is not True):
        tf.config.set_visible_devices([], 'GPU')

    # load data and evaluate
    dataset_warpper = NN_load_datasets.load_dataset_model_type(seq_path=seq_path, labels_path_minus=labels_path_minus,
                                                               labels_path_plus=labels_path_plus,
                                                               model_type=model_type, data_type=data_type,
                                                               only_predict=only_predict, split=split,
                                                               index_for_split=index_for_split)
    if (split):
    ...
    for i in range(model_num):
        predicted = predict_and_evaluate(model_list[i], test_set_wrapper, model_type, data_type, [],
                                         compute_mean_and_std, only_predict=True)
        test_predicted = predicted if i == 0 else test_predicted + predicted
        if validation_seq_wrapper is not None:
            predicted = predict_and_evaluate(model_list[i], validation_seq_wrapper, model_type, data_type, [],
                                             compute_mean_and_std, only_predict=True)
    ...
    if (only_predict):
        ...
        np.savetxt(output_path + 'predictions_' + model_id + '_' + model_type + '_' + data_type + '.csv',
    ...
    print('\033[1m' + "test evaluation" + '\033[0m')
    results_dict['test'] = predict_and_evaluate(None, test_set_wrapper, model_type, data_type, [], compute_mean_and_std,
    ...
    if validation_seq_wrapper is not None:
        ...
        results_dict['validation_seq'] = predict_and_evaluate(None, validation_seq_wrapper, model_type, data_type, [],
                                                              compute_mean_and_std, only_evaluate=validation_predicted)
...
def evaluate_IG_test(model_path, validate_seq_path, validate_labels_path_minus,
    ...
    for target_range_item in target_range_list:
        NN_utilies.preform_compere_IG_tests(validate_seq_path=validate_seq_path,
                                            ...
                                            model_type_list=model_type_list, model_id_list=model_id_list,
                                            data_type_list=data_type_list,
                                            target_range_list=[target_range_item], output_pdf=output_pdf,
                                            pdf_dir=pdf_dir)
...
def evaluate_TF_modisco(model_path, seq_path, labels_path_minus, labels_path_plus,
    ...
    for i in range(len(target_range_list)):
        if (IG_attributions_path is None):
            [hyp_impscores, impscores, onehot_data, null_distribution] = TF_modisco.Generate_modisco_Dataset(model_path,
                                                                                                             seq_path,
                                                                                                             labels_path_minus,
                                                                                                             labels_path_plus,
                                                                                                             model_id,
            ...
            [hyp_impscores, impscores, onehot_data, null_distribution] = general_utilies.pickle.load(
        ...
        zipf = TF_modisco.zipfile.ZipFile(zip_name_initial + datetime.datetime.now().strftime(
            "%Y%m%d-%H%M%S") + '_' + model_id + '_' + data_type + '_' + str(
            'all_outputs' if target_range_list[i] is None else target_range_list[
                i]) + '_' + test_validation_train_or_all_set + '_set.zip', 'w', TF_modisco.zipfile.ZIP_DEFLATED)
...
def evaluate_RESA_with_linear_model(model_path, data_type, padding=False, aggregation_type="mean"):
            ...
            for i in range(split_seq_to_chunks_futures_array.shape[1]):
                ...
                if (i == 0):
                    if (num_of_outputs == 1):
                        ...
                        preticted_values_matrix = [np.expand_dims(preticted[:, 0], axis=1),
                    ...
                    if (num_of_outputs == 1):
                        ...
                        preticted_values_matrix = [
                            np.concatenate((preticted_values_matrix[0], np.expand_dims(preticted[:, 0], axis=1)),
                                           axis=1),
                            np.concatenate((preticted_values_matrix[1], np.expand_dims(preticted[:, 1], axis=1)),
                                           axis=1)]
    ...
    if (num_of_outputs == 2):
        ...
        print("(pearson, p-value):",
              stats.pearsonr(predicted_values.reshape((25043,)), observed_values.reshape((25043,))))
#####################################################################################
